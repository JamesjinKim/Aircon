...
#### **AUTO 설정 저장 시 PM25SET 응답 누락 문제**
- **문제**: 설정값 저장 시 4개 명령어가 딜레이 없이 연속 전송되어 PM25SET ACK 응답이 씹히는 현상
- **원인**: 장비 펌웨어에서 응답 처리 중 다음 명령어가 들어와 응답 잼 발생
- **해결**: 명령어 간 200ms 딜레이 적용으로 순차 전송

### 🔧 **변경 사항**

#### **auto_manager.py**
- `handle_save()`: 명령어 대기열 방식으로 변경
- `_send_next_save_command()`: QTimer를 사용한 200ms 간격 순차 전송 구현
- 전송 순서: TEMPSET → CO2SET → PM25SET → SEMITIME (각 200ms 간격)

### 📝 **시리얼 통신 주의사항**
- 여러 명령어 연속 전송 시 **최소 200ms 간격** 유지 필요
- ACK 응답이 필요한 설정 명령어는 순차 전송 필수

### 📁 **수정된 파일**
- `managers/auto_manager.py`
